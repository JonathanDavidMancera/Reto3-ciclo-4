$(document).ready( function () {
    ...
    //localStorage.setItem('productsOrder', '');
    ...
    $.ajax({
        ...
        success: function(response){
            if(response.length > 0){
                ...
                var btnAdd = '', btnInfo = '', btnDelect = '';
                for (var a = 0; a < response.length; a++) {
                    
                    btnAdd = "<a class='btn btn-success text-center text-white' id='btnAdd' style='cursor: pointer;' onclick='javascript:add_orden(\""+response[a]['reference']+"\")'><i class='fas fa-plus fa-1x'></i> Añadir</a>";
                    btnInfo = "<a class='btn btn-info text-center text-white' id='btnInfo' style='cursor: pointer;' onclick='javascript:ver_detalle(\""+response[a]['reference']+"\")'><i class='fas fa-eye fa-1x'></i> Ver</a>";
                    btnDelect = "<a class='btn btn-danger text-center text-white' id='btnDelect' style='cursor: pointer;' onclick='javascript:eliminar_product(\""+response[a]['reference']+"\")'><i class='fas fa-trash-alt fa-1x'></i> Eliminar</a>";
                    
                    if(response[a]['availability'] !== true || response[a]['quantity'] < 0){
                        btnAdd = "";
                    }
 ...
 * add_orden()
 * @param reference, Referencia del producto
 * @return, No retorna nada, solo agrega productos a la orden
 */
function add_orden(reference){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            const validacion = productos.find(product => product.reference === reference);
            
            if(validacion === undefined){
                productos.push({reference: reference, quantity: 1});
            }else{
    ...
    localStorage.setItem('productsOrder', JSON.stringify(productos));
    
    const productsOrder = localStorage.getItem('productsOrder');
    console.log(productsOrder);
    
    if(val !== false && productsOrder !== '[]'){
        alert("El producto con referencia: "+reference+" se agregó correctamente a la orden");
    }
}
