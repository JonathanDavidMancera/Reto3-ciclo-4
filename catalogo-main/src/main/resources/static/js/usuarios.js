            ...
            if(response.length > 0){
                for (var a = 0; a < response.length; a++) {
                    tabla.row.add([
                        ...
                        '<a class="btn btn-danger text-center text-white" style="cursor: pointer;" onclick="javascript:eliminar_user('+response[a]['id']+')"><i class="fas fa-trash-alt fa-1x"></i> Eliminar</a>'
                    ]).draw();
                }
            }
        }
...
function registrar_user(){
    ...
    $.ajax({
        method: "GET",
        url: url_pet+"/api/user/emailexist/"+email_user,
        success: function(response){
            if(response === false){
                $.ajax({
                    method: "POST",
